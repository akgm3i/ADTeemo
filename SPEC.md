...
- **APIレスポンス指針**: 成否判定はHTTPステータスコードを唯一のソースとし、`success` のような重複したフラグはレスポンスに含めない。成功時のボディは必要最小限のデータに絞り、エラー時は適切なステータスとエラーメッセージ文字列を返す。この方針はBot側のHTTPクライアントおよびテストコードでも前提とする。
