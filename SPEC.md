...
- **指定メンバーの継続試合監視**: `/watch-match @member` により、Riot ID 連携済みの Discord メンバーをギルド単位で継続監視できる。監視は `/unwatch-match @member` で解除されるまで続き、ノーマル、ランク、カスタムなど queue 種別では絞り込まない。
  ...
  - **同一試合の通知統合**: 同じ `guildId + channelId + Riot platform + gameId` で複数の監視対象が同一試合に入っている場合、試合中通知は 1 つの Discord 投稿へ統合し、対象者一覧を表示する。Riot platform が異なるアカウントでは numeric `gameId` が同じでも別試合として扱い、共有投稿を統合しない。複数対象の統合投稿では対象者ごとのチャンピオンを表示し、単一対象の Riot ID を代表値として footer に出さない。
  - **共有投稿 ID の保持と復旧**: 統合された試合中投稿の message ID は、その試合の active watcher 全員が保持する。Discord 側で投稿が削除され編集から新規送信へ fallback した場合や、過去の送信失敗などで一部 watcher が message ID を持たない状態から初めて共有投稿 ID が確立した場合も、同じ active group の watcher へ置換後または新規確立した message ID を同期する。進捗通知の間隔内で Discord 投稿の編集を省略する場合も、既に group で共有投稿 ID を復元できているなら未同期 watcher へ保存する。開始通知または gameId 変更時の新試合開始通知で共有投稿を編集した場合も、その編集時刻を group の通知時刻として同期し、同一 tick の後続 watcher が進捗通知で開始通知を上書きしないようにする。
  ...
  - **結果通知の非統合**: 試合結果は監視対象ごとの個別通知として扱い、同一試合の複数対象を 1 つの結果投稿へ統合しない。共有された試合中投稿 ID を結果通知に使う場合は同一 active group 内で 1 回だけ再利用し、後続の結果通知は新規投稿へ分離して上書きを防ぐ。PR #43 導入前または移行途中の個別・複数共有試合中投稿 ID が残る状態では、distinct な既存 active 投稿 ID ごとに 1 回だけ結果通知へ変換する。旧 `FETCHING_RESULT` 形式で `currentMatchId` と `currentNotificationMessageId` に結果取得待ちを保持している互換状態も、新しい pending result fields と同じく使用済み結果投稿 ID として扱う。
...
- **メッセージカタログ**: Bot の表示文言は言語・テーマ別のメッセージカタログで管理する。テーマ別カタログにキーがない場合は既定テーマへ fallback するが、JSON 内の重複キーは後勝ちで文言が失われるため許容しない。
