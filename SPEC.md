...
PR #39で `CS/min` と `キル関与率` は実装済みである。残件はLP delta、外部戦績ページリンク、試合中の各プレイヤーロール表示である。現行のMatch-v5 / Spectator-v5だけでは不足する情報があるため、League-v4や試合前後スナップショット、リンク生成helper、別データ源の検討が必要である。
