...
- Backend API routeはpublic、browser callback、Bot serviceのいずれかに分類し、Bot service routeには専用Bearer credentialの認証を一括適用する。Discord token、Riot API key、RSO secretは流用しない。詳細は [ADR 0001](./docs/adr/0001-bot-service-authentication.md) を参照する。
- 本番Backend APIのhost portはloopbackだけへbindし、外部公開が必要なbrowser callbackはTLS reverse proxyで対象pathだけを転送する。
- API / Botの本番ログはstdoutの1行JSONを正本とし、共通fieldとERROR分類・相関IDを持たせる。秘密値、Riot識別子、Discord user ID、SQL parameterはnested contextでも記録せず、opaqueなError本文を解析・保存しない。責務境界は [ADR 0002](./docs/adr/0002-structured-logging-trust-boundary.md) を参照する。
- APIは検証済み`X-Correlation-ID`をresponseへ引き継ぎ、欠落・不正時はUUIDを生成する。request pathは実値ではなくroute templateを記録する。
- DB query logは既定OFFとし、明示opt-inかつDEBUG時もSQL templateだけを記録してparameterを破棄する。
