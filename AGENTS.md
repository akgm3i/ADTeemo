...
- 作業開始時は現在ブランチと未コミット差分を確認し、依頼内容に応じた作業ブランチを作成または切り替える。
- 変更完了後、品質確認を行い、依頼範囲の差分だけを日本語Conventional Commits形式でコミットする。
- push / PR作成はユーザーの明示依頼がある場合のみ行う。
- 既存の未追跡ファイルや依頼外の差分はコミットに含めない。
