...
- Type: proposal
- Status: superseded
- Summary: 採用前の比較記録。決定はADR 0006と現行ガイドへ移管。
- Read when: 監視対象ポリシー、アカウント複数化、既存watcher移行を検討するとき。
- Related: [#34](https://github.com/akgm3i/ADTeemo/issues/34), [#48](https://github.com/akgm3i/ADTeemo/issues/48), [#89](https://github.com/akgm3i/ADTeemo/issues/89)
- Code: [watcher schema](../../api/src/db/schema.ts)
- Tests: [watcher route tests](../../api/src/routes/match_watchers.test.ts)
- Reviewed: 2026-09-25
- Verified: 2026-09-25、採用内容をADRと実装へ移管。以下の候補・未決定事項は過去の検討記録。

2026-09-25の3iの指定に基づき、全登録account監視・guildとDiscordユーザー単位opt-out・カスタムゲームはメイン1件を採用した。[ADR 0006](../adr/0006-account-monitoring-policy.md)、[現在の操作](../user/riot-accounts-and-monitoring.md)、[移行と運用](../integrations/account-monitoring.md)を正とする。以下は採用前の比較記録であり、候補の列定義や未決定事項を現行仕様として扱わない。
...
- 手動監視を維持: 明示的な対象・通知先の選択を保てる。登録の手間と監視漏れが残る。
- 登録済みユーザーの既定監視: 登録の手間を減らせる。本人への説明、opt-out、通知先・負荷上限の設計が必要。
- 管理者によるguild opt-inと本人opt-outの併用: サーバー側の準備を確認できるが、設定状態が増える。

複数アカウント時のmainのみ/全アカウント、opt-outのuser/account単位は[#89](https://github.com/akgm3i/ADTeemo/issues/89)で合わせて判断する。migration・route・command・テストの作業管理は関連Issueへ集約し、この文書へ進捗を複製しない。
