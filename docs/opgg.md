...
- Action ID:
  ...
  - 保持済みAction IDで、404かつレスポンスがServer Action不在を示す場合だけ、再抽出して1回だけ再試行する。
  - 403 / 429、timeout、通信失敗、5xxでは再抽出・再送せずfallbackする。
- fixture:
  - OP.GGのHTML全体ではなく、Action ID抽出に必要な最小HTML/chunk断片をfixture化する。
  - `getGames` / `getGame` は表示と照合に必要な最小レスポンスをfixture化する。
- 出典表示:
  - OP.GG由来情報はOP.GG試合詳細リンクと一緒に表示するため、追加の出典表記は行わない。
- 追加抑制:
  - 初期実装ではギルド単位・ユーザー単位の永続抑制は行わない。
  - 実運用でプロセス内抑制だけでは不十分と分かった場合に、永続抑制を再検討する。
...
保持済みAction IDで、404かつレスポンスがServer Action不在を示す場合、OP.GG側のビルド更新を疑い、プロフィールHTMLとchunkを再取得してAction IDを再抽出します。再抽出後に同じOP.GG操作を1回だけ再試行し、それでも失敗する場合はOP.GG詳細リンクとOP.GG由来情報を省略します。通常の404、403 / 429、timeout、通信失敗、5xxはAction ID失効と区別し、追加アクセスを避けるため再抽出・再送せずfallbackします。
...
2. OP.GG連携の初回実行時、または保持済みAction IDで404かつレスポンスがServer Action不在を示す場合に、プロフィールHTMLとchunkからServer Action IDを抽出して保持する。
...
- `OPGG_ENABLED`: Backend APIによるOP.GG試合詳細リンク解決、必要時の `renewal`、OP.GG詳細情報取得を有効にする。Botはこの設定を参照しない。

`OPGG_ENABLED` が有効でない場合、Backend APIはOP.GGへHTTPリクエストを送らず、BotはOP.GG情報なしで既存の試合結果通知だけを継続します。

Server Action ID用の環境変数は設けません。Action IDは初回実行時にOP.GGから抽出して保持し、404かつレスポンスがServer Action不在を示す場合だけ再抽出します。
