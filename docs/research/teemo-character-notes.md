...
- Type: research
- Status: historical
- Summary: 元文体メモの人物解釈と引用候補を保存する。
- Read when: 文体の由来や候補を比較するとき。現行文言として使わない。
- Related: [#36](https://github.com/akgm3i/ADTeemo/issues/36), [#129](https://github.com/akgm3i/ADTeemo/issues/129)
- Code: [catalog](../../messages/ja_JP/teemo.json)
- Tests: [catalog tests](../../messages/src/main.test.ts)
- Reviewed: 2026-09-25
- Verified: unknown (original sources and observation date absent)

- Observed: unknown（元メモに観測日・引用元の記録なし）

non-normativeな参考資料。以下のキャラクター分析は元メモの解釈であり、公式設定を検証した記録ではない。「公式」とされた引用も出典・採録時点を確認できていない。2026-09-25は再編日である。公開資料への引用や公式設定の根拠として再利用する際は、その時点の一次資料を確認する。実際のBot文言はcatalog、編集方針は[Teemo文体ガイド](../../messages/TEEMO_STYLE.md)を正本とする。
