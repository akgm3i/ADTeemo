...
36. [ ] Riot API 連携でゲーム開始検知・戦績取得・内部レート更新を自動化する（`bot/src/features/match_tracking.ts` など）。
    ...
    - [x] 同一 `guildId + channelId + Riot platform + gameId` の複数監視対象を1つの試合中通知へ統合し、対象者一覧を表示する。同じ numeric `gameId` でも Riot platform が異なる場合は別試合として分離する。
    - [x] 統合された試合中投稿 ID を active watcher 全員へ保持・同期し、投稿削除後の置換、送信失敗後の初回 ID 確立、進捗編集を省略する tick での未同期 ID 補正、代表 watcher 無効化後も既存投稿の編集を継続できるようにする。
    - [x] 統合された試合中投稿の進捗通知間隔を共有投稿単位で判定し、開始通知または gameId 変更時の新試合開始通知で共有投稿を編集した tick でも、後続 watcher が進捗通知で同じ投稿を再編集しないようにする。
    - [x] 同一試合の結果通知は監視対象ごとの個別通知として扱い、共有試合中投稿の上書きを防ぎつつ、統合前の個別投稿 ID、複数の共有投稿 ID、旧 `FETCHING_RESULT` 形式が残る移行状態にも対応する。
...
43. [x] ティーモ版メッセージの不足キーを補い、`check:messages` で現行ディレクトリの不足キー・重複キー整合性を検知する。
